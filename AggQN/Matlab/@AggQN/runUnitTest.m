...
switch number
  ...
  case 1
    ...
    for k = 1:size(AQN.Y,2)-AQN.j
      ...
      % Print error
      if AQN.verbosity >= 1
        fprintf('AggQN: Error in computation of a_{%4d,1}     : %e\n',k,norm(AQN.Q(:,AQN.j:AQN.j+k-1)'*AQN.HQ_j*ak + AQN.b_j(k)*AQN.S(:,AQN.j:AQN.j+k-1)'*AQN.y_j));
      end
  ...
  case 5
    ...
    % Compute Ytilde
    Ytilde = AQN.Y;
    Ytilde(:,AQN.j:end-1) = AQN.Y(:,AQN.j:end-1) + AQN_H.hessian*AQN.Q*AQN.A_j + AQN.y_j*AQN.b_j';
    ...
    matrix3 = abs((Ytilde(:,AQN.j:end) - AQN.Y(:,AQN.j:end))'*inv(AQN_H.hessian)*(Ytilde(:,AQN.j:end)-AQN.Y(:,AQN.j:end)) - ...
              ...
              A'*AQN.C'*tril(AQN.SY(AQN.j:end,AQN.j:end),-1) + ...
              tril(AQN.SY(AQN.j:end,AQN.j:end),-1)'*AQN.C*A);
